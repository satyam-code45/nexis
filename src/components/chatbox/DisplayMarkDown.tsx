...
export const DisplayImageMarkdownURL = memo(function MarkdownBubble({
  text,
}: {
  text: string;
}) {
...
export const DisplayMarkDown = memo(function MarkdownBubble({
  text,
}: {
  text: string;
}) {
  return (
    <ReactMarkdown
      ...
      components={{
        ...
        pre: ({ node, ...props }) => (
          <pre
            className="p-3 rounded-lg overflow-x-auto bg-black/10 dark:bg-white/10"
            {...props}
          />
        ),
        ...
        table: ({ node, ...props }) => (
          <table
            className="table-auto border-collapse border border-gray-300 w-full mb-5"
            {...props}
          />
        ),
        thead: ({ node, ...props }) => (
